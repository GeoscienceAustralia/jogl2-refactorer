...
class VBORenderer implements ActionListener, ChangeListener, GLEventListener
{
    ...
    public void actionPerformed(ActionEvent e)
    {
        ...
        {
            if (glDisplay.getBufferDataUsage() != GL2.GL_STATIC_DRAW)
            {
                glDisplay.setBufferDataUsage(GL2.GL_STATIC_DRAW);
                usageChanged = true;
            }
        }
        else if (action.equalsIgnoreCase("Dynamic"))
        {
            if (glDisplay.getBufferDataUsage() != GL2.GL_DYNAMIC_DRAW)
            {
                glDisplay.setBufferDataUsage(GL2.GL_DYNAMIC_DRAW);
                usageChanged = true;
            }
        }
        else if (action.equalsIgnoreCase("Stream"))
        {
            if (glDisplay.getBufferDataUsage() != GL2.GL_STREAM_DRAW)
            {
                glDisplay.setBufferDataUsage(GL2.GL_STREAM_DRAW);
                usageChanged = true;
            }
        }
    }
    ...
    public void init(GLAutoDrawable drawable)
    {
        ...
        gl.glHint(GL2.GL_PERSPECTIVE_CORRECTION_HINT,
            GL2.GL_NICEST);        // Set Perspective Calculations To Most Accurate
    ...
    public void display(final GLAutoDrawable drawable)
    {
        ...
        for (int i = 0; i < iterationCount; i++)
        {
            ...
            if (glDisplay.getUseFloats())    // float version
            {
                ...
                // Render the mesh
                mesh.render(gl, GL2.GL_FLOAT, Buffers.SIZEOF_FLOAT, mesh.vertices, mesh.texCoords);
            }
            else                            // double version
            {
                // Move The Camera
                ...
                // Render the mesh
                mesh.render(gl, GL2.GL_DOUBLE, Buffers.SIZEOF_DOUBLE, mesh.verticesDbl, mesh.texCoordsDbl);
            }
        }
    ...
    private class Mesh
    {
        ...
        public boolean loadHeightmap(GL2 gl, String szPath, float flHeightScale, float flResolution,
        ...
        public boolean loadHeightmap(GL2 gl, String szPath, double dblHeightScale, double dblResolution,
            boolean VBOsupport, int bufferTypeSize) throws IOException
        {
            TextureReader.Texture texture = null;
            texture = TextureReader.readTexture(szPath);
            ...
            texCoordsDbl = Buffers.newDirectDoubleBuffer(vertexCount * 2);                     // Allocate Tex Coord Data
        ...
        public boolean loadTexture(GL2 gl, TextureReader.Texture texture)
        {
            // Load The Texture Into OpenGL
            gl.glGenTextures(1, textureId, 0);                            // Get An Open ID
            gl.glBindTexture(GL2.GL_TEXTURE_2D, textureId[0]);             // Bind The Texture
            gl.glTexImage2D(GL2.GL_TEXTURE_2D, 0, 3, texture.getWidth(), texture.getHeight(), 0, GL2.GL_RGB,
        ...
        public void render(GL2 gl, int glDataType, int bufferTypeSize, Buffer vertexBuffer, Buffer textureBuffer)
        {
            // Enable Pointers
            ...
            if (glDisplay.getUseVBOs())
            {
                ...
                if (glDisplay.getReloadNow())
                {
                    ...
                    gl.glBufferSubData(GL2.GL_ARRAY_BUFFER, 0, vertexCount * 3 * bufferTypeSize, vertexBuffer);
                }
                ...
                {   // copy textureCoords after vertices
                    gl.glBufferSubData(GL2.GL_ARRAY_BUFFER, vertexCount * 3 * bufferTypeSize,
                ...
                if (glDisplay.getUseTriangleStrip())
                {
                    gl.glBindBuffer(GL2.GL_ELEMENT_ARRAY_BUFFER, mesh.VBOIndices[0]);
                ...
                if (glDisplay.getUseTriangleStrip())       // draw using GL_TRIANGLE_STRIP
                    gl.glDrawElements(GL2.GL_TRIANGLE_STRIP, mesh.indices.limit(), GL2.GL_UNSIGNED_INT, 0);
            ...
            else
            {
                ...
                // Render
                if (glDisplay.getUseTriangleStrip())       // draw using GL_TRIANGLE_STRIP
                    gl.glDrawElements(GL2.GL_TRIANGLE_STRIP, mesh.indices.limit(), GL2.GL_UNSIGNED_INT, mesh.indices);
                else                                     // draw using GL_TRIANGLES
                    gl.glDrawArrays(GL2.GL_TRIANGLES, 0, mesh.vertexCount);    // Draw All Of The Triangles At Once
            }
        ...
        private void buildVBOs(GL2 gl, int bufferTypeSize, Buffer vertexBuffer, Buffer textureBuffer)
        {
            ...
            if (glDisplay.getUseTriangleStrip())
            {
                ...
                gl.glGenBuffers(1, VBOIndices, 0);                                       // Get A Valid Name
                gl.glBindBuffer(GL2.GL_ELEMENT_ARRAY_BUFFER, VBOIndices[0]);          // Bind The Buffer
                gl.glBufferData(GL2.GL_ELEMENT_ARRAY_BUFFER, indexCount * Buffers.SIZEOF_INT,
