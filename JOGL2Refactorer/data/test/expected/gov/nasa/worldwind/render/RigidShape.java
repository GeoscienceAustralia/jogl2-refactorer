...
public abstract class RigidShape extends AbstractShape
{
    ...
    protected void doDrawOutline(DrawContext dc)
    {
        for (int i = 0; i < getFaceCount(); i++)
        {
            ...
            mesh.setMode(Geometry.ELEMENT, GL2.GL_LINES);
            ...
            mesh.setMode(Geometry.ELEMENT, GL2.GL_TRIANGLES);
        }
    }
        ...
        for (int i = 0; i < getFaceCount(); i++)
        {
                ...
                dc)) // bind initiates retrieval
            {
                ...
                if (getOffsets(i, 0) != null)   // if texture offsets exist
                {
                    ...
                    gl.glTexCoordPointer(2, GL2.GL_FLOAT, 0, offsetCoords.rewind());
                }
                else
                {
                    gl.glTexCoordPointer(2, GL2.GL_FLOAT, 0, mesh.getBuffer(Geometry.TEXTURE).rewind());
                }
    ...
    protected void fillVBO(DrawContext dc)
    {
        ...
        if (shapeData.refillIndexVBO)
        {
            try
            {
                for (int face = 0; face < getFaceCount(); face++)
                {
                    ...
                    gl.glBindBuffer(GL2.GL_ELEMENT_ARRAY_BUFFER, vboIds[2 * face + 1]);
                    gl.glBufferData(GL2.GL_ELEMENT_ARRAY_BUFFER, ib.limit() * Buffers.SIZEOF_FLOAT, ib.rewind(),
                        GL2.GL_DYNAMIC_DRAW);
                }
        ...
        try
        {
            for (int face = 0; face < getFaceCount(); face++)
            {
                ...
                gl.glBindBuffer(GL2.GL_ARRAY_BUFFER, vboIds[2 * face]);
                gl.glBufferData(GL2.GL_ARRAY_BUFFER, vb.limit() * Buffers.SIZEOF_FLOAT, vb.rewind(),
                    GL2.GL_STATIC_DRAW);
            }
        }
        finally
    ...
    public List<Intersection> intersectFace(Line line, int index, Matrix renderMatrix) throws InterruptedException
    {
        ...
        List<Intersection> intersections = Triangle.intersectTriangleTypes(localLine, vertices,
            (IntBuffer) mesh.getBuffer(Geometry.ELEMENT), GL2.GL_TRIANGLES);
