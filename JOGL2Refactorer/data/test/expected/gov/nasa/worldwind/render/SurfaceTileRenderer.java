...
public abstract class SurfaceTileRenderer implements Disposable
{
    ...
    public void dispose()
    {
        ...
        if (this.alphaTexture != null)
            this.alphaTexture.destroy(GLContext.getCurrent().getGL().getGL2());
        this.alphaTexture = null;
        if (this.outlineTexture != null)
            this.outlineTexture.destroy(GLContext.getCurrent().getGL().getGL2());
    ...
    public void renderTiles(DrawContext dc, Iterable<? extends SurfaceTile> tiles)
    {
        ...
        GL2 gl = dc.getGL();
        int alphaTextureUnit = GL2.GL_TEXTURE1;
        ...
        try
        {
            ...
            {
                numTexUnitsUsed = 3;
                alphaTextureUnit = GL2.GL_TEXTURE2;
    ...
    protected void initAlphaTexture(int size)
    {
        ...
        TextureData textureData = new TextureData(GLProfile.get(GLProfile.GL2), GL2.GL_ALPHA, size, size, 0, GL2.GL_ALPHA,
            GL2.GL_UNSIGNED_BYTE, false, false, false, textureBytes.rewind(), null);
        this.alphaTexture = TextureIO.newTexture(textureData);

        this.alphaTexture.bind(GLContext.getCurrent().getGL().getGL2());
        this.alphaTexture.setTexParameteri(GLContext.getCurrent().getGL().getGL2(), GL2.GL_TEXTURE_MAG_FILTER, GL2.GL_NEAREST);
        this.alphaTexture.setTexParameteri(GLContext.getCurrent().getGL().getGL2(), GL2.GL_TEXTURE_MIN_FILTER, GL2.GL_NEAREST);
        this.alphaTexture.setTexParameteri(GLContext.getCurrent().getGL().getGL2(), GL2.GL_TEXTURE_WRAP_S, GL2.GL_CLAMP_TO_BORDER);
        this.alphaTexture.setTexParameteri(GLContext.getCurrent().getGL().getGL2(), GL2.GL_TEXTURE_WRAP_T, GL2.GL_CLAMP_TO_BORDER);
    ...
    protected void initOutlineTexture(int size)
    {
        ...
        TextureData textureData = new TextureData(GLProfile.get(GLProfile.GL2), GL2.GL_LUMINANCE, size, size, 0, GL2.GL_LUMINANCE,
            GL2.GL_UNSIGNED_BYTE, false, false, false, textureBytes.rewind(), null);
        this.outlineTexture = TextureIO.newTexture(textureData);

        this.outlineTexture.bind(GLContext.getCurrent().getGL().getGL2());
        this.outlineTexture.setTexParameteri(GLContext.getCurrent().getGL().getGL2(), GL2.GL_TEXTURE_MAG_FILTER, GL2.GL_NEAREST);
        this.outlineTexture.setTexParameteri(GLContext.getCurrent().getGL().getGL2(), GL2.GL_TEXTURE_MIN_FILTER, GL2.GL_NEAREST);
        this.outlineTexture.setTexParameteri(GLContext.getCurrent().getGL().getGL2(), GL2.GL_TEXTURE_WRAP_S, GL2.GL_CLAMP_TO_EDGE);
        this.outlineTexture.setTexParameteri(GLContext.getCurrent().getGL().getGL2(), GL2.GL_TEXTURE_WRAP_T, GL2.GL_CLAMP_TO_EDGE);
    }
}
