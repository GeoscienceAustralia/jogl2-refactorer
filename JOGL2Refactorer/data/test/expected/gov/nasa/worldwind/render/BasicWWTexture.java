...
public class BasicWWTexture implements WWTexture
{
    ...
    protected Texture initializeTexture(DrawContext dc, Object imageSource)
    {
        ...
        if (imageSource instanceof String)
        {
            ...
            try
            {
                TextureData td = TextureIO.newTextureData(GLProfile.get(GLProfile.GL2), (InputStream) streamOrException, this.useMipMaps, null);
        ...
        else if (imageSource instanceof BufferedImage)
        {
            try
            {
                TextureData td = AWTTextureIO.newTextureData(GLProfile.get(GLProfile.GL2), (BufferedImage) imageSource, this.useMipMaps);
                t = TextureIO.newTexture(td);
                haveMipMapData = td.getMipmapData() != null;
            }
            catch (Exception e)
        ...
        else if (imageSource instanceof URL)
        {
            try
            {
                ...
                TextureData td = TextureIO.newTextureData(GLProfile.get(GLProfile.GL2), stream, this.useMipMaps, null);
                t = TextureIO.newTexture(td);
                haveMipMapData = td.getMipmapData() != null;
            }
            catch (Exception e)
        ...
        gl.glTexParameteri(GL2.GL_TEXTURE_2D, GL2.GL_TEXTURE_MIN_FILTER,
            useMipMapFilter ? GL2.GL_LINEAR_MIPMAP_LINEAR : GL2.GL_LINEAR);
